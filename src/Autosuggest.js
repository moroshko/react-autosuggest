...
const defaultRenderSuggestionsContainer = ({ containerProps, children }) => <div {...containerProps}>{children}</div>;
...
export default class Autosuggest extends Component {
    ...
    shouldComponentUpdate = (nextProps, nextState) =>
        ...
        nextState.highlightedSuggestionIndex !== this.state.highlightedSuggestionIndex;

    componentDidUpdate(prevProps, prevState) {
        const { suggestions, onSuggestionHighlighted, highlightFirstSuggestion } = this.props;

        if (!shallowEqualArrays(suggestions, prevProps.suggestions) && suggestions.length > 0 && highlightFirstSuggestion) {
            this.highlightFirstSuggestion();
            return;
        }
    ...
    getSuggestionIndices(suggestionElement) {
        ...
        return {
            sectionIndex: typeof sectionIndex === 'string' ? parseInt(sectionIndex) : null,
    ...
    onSuggestionSelected = (event, data) => {
        const { alwaysRenderSuggestions, onSuggestionSelected, onSuggestionsFetchRequested, keepSuggestionsOnSelect, inputProps: { value } } = this.props;
    ...
    onSuggestionClick = (event) => {
        const { alwaysRenderSuggestions, focusInputOnSuggestionClick, keepSuggestionsOnSelect } = this.props;
    ...
    render() {
        ...
        const { isFocused, isCollapsed, highlightedSectionIndex, highlightedSuggestionIndex, valueBeforeUpDown } = this.state;
        ...
        const autowhateverInputProps = {
            ...
            onChange: (event) => {
                const { value } = event.target; //eslint-disable-line
                const shouldRender = shouldRenderSuggestions(value);
